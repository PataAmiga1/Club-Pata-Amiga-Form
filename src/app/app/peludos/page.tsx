...
export default async function PeludosPage({
  ...
  const [{ data: pets }, { data: appeals }] = await Promise.all([
    supabase
      .from("pets")
      .select(
        "id, name, species, breed, age_years, age_months, photo_url, approval_status, waiting_period_end_date, waiting_period_start_date, waiting_period_bypassed, created_at, is_active, deactivation_reason, deactivated_at",
      )
      .eq("user_id", user.id)
